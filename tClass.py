...
THESESSIONID="8a0c97d1a4024ed9941a78b69860defa"
...
BASE_COOKIES={"EDUWEBDEVICE":"6ad321f5006a415580593979ff410de6",
		...
		"bpmns":"1",
		"WM_NI":"yBXgGyOb5nQB68fqY4Gn7vhHz28OTsKHFkffW2Wa%2B11kalBBkdHDSMG9w94PI%2FTASA0zfhGAtwY5pD%2FX05Zuu7RW75g5pIzK9M1eKHwtK4JRYbSIT07EB7032zmiv6ZmUHk%3D",
		...
		"__utmc":"63145271",
		"STUDY_SESS":"",
		"STUDY_INFO":"UID_778EEE8FE8ED1F4D510F4E4BA7B3C4A4|4|1036064017|1552116833743",
...
def getCourseOfSubject(subjectId):
	...
	for course in coursesDict["result"]:
		...
		querySql="SELECT * FROM course WHERE course_id=%s"
		
		# 查询返回 记录数
		resCount=courseCursor.execute(querySql,course["currentTermId"])
		if(resCount==0):
	...
	for index in range(2,totalPageCount+1):
		...
		for course in coursesDict["result"]:
			...
			if(resCount==0):
				...
				#3 根据id ,  还得构造一个name ， 学校简称（上面数据里有）-inschoolid  这个 -可能有问题
				referName=(school["shortName"])+"-"+str(course["id"])
				print("----------获取课程   "+course["name"]+"  课件资源")
				getSource(course["currentTermId"],referName)
